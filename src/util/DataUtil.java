...
public class DataUtil {
	...
	public static List<Entry<String, Integer>> convertMapToSortedList(
		...
		for (Entry<String, Integer> en : map.entrySet()) {
			...
			for (int i = 0; i < list.size(); i++) {
				if (en.getValue().intValue() > list.get(i).getValue()
						.intValue()) {
					pos = i;
					break;
				}
	...
	public static List<Entry<String, Double>> convertMapToSortedListInDouble(
			Map<String, Double> map) {
		List<Entry<String, Double>> list = new ArrayList<Entry<String, Double>>();
		for (Entry<String, Double> en : map.entrySet()) {
			int pos = list.size();
			for (int i = 0; i < list.size(); i++) {
				if (en.getValue().doubleValue() > list.get(i).getValue()
						.doubleValue()) {
					pos = i;
					break;
				}
	...
	public static List<Entry<String, Double>> convertMapToSortedListInDoubleWithThreshold(
			HashMap<String, Double> map, double scoreThreshold) {
		List<Entry<String, Double>> list = new ArrayList<Entry<String, Double>>();
		for (Entry<String, Double> en : map.entrySet()) {
			if (en.getValue() < scoreThreshold) {
				continue;
			}
			int pos = list.size();
			for (int i = 0; i < list.size(); i++) {
	...
	public static List<Entry<Integer, Double>> convertMapInIntegerToSortedListInDouble(
	...
	public static List<Entry<Integer, Double>> convertMapInIntegerToSortedListInDoubleWithThreshold(
			HashMap<Integer, Double> map, double scoreThreshold) {
		List<Entry<Integer, Double>> list = new ArrayList<Entry<Integer, Double>>();
		for (Entry<Integer, Double> en : map.entrySet()) {
			if (en.getValue() < scoreThreshold) {
				continue;
	...
	public static List<Entry<String, Double>> topNInMap2List(
			HashMap<String, Double> map, int n) {
		List<Entry<String, Double>> list = new ArrayList<Entry<String, Double>>();
		if (map.size() < n) {
			list.addAll(map.entrySet());
			return convertMapToSortedListInDouble(map);
		} else {
			int pos, num;
			for (Entry<String, Double> en : map.entrySet()) {
				pos = list.size();
	...
	public static List<Entry<Integer, Double>> topNInMap2List2(
			HashMap<Integer, Double> map, int n) {
		List<Entry<Integer, Double>> list = new ArrayList<Entry<Integer, Double>>();
		if (map.size() < n) {
			list.addAll(map.entrySet());
			return convertMapInIntegerToSortedListInDouble(map);
		} else {
			int pos, num;
			for (Entry<Integer, Double> en : map.entrySet()) {
				pos = list.size();
	...
	public static List<Entry<Integer, Double>> topSmallNInMap2List2(
			HashMap<Integer, Double> map, int n) {
		List<Entry<Integer, Double>> list = new ArrayList<Entry<Integer, Double>>();
		if (map.size() < n) {
			list.addAll(map.entrySet());
